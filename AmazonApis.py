...
class AmazonApis:
    ...
    def _get_current_user_id(self) -> Union[str, None]:
        """
        Returns the currently logged-in user ID, or None if no user is logged in.
        """
        return self.state.get("current_user")

    def _require_login(self) -> Dict[str, Union[bool, str]]:
        """
        Checks if a user is logged in. Returns error dict if not logged in, None if logged in.
        """
    ...
    def logout_user(self) -> Dict[str, Union[bool, str]]:
        ...
        Returns:
            Dict: A dictionary containing logout status and message.
        """
        if not self._get_current_user_id():
    ...
    def add_payment_card(
        ...
        """
        Adds a new payment card to the currently logged-in user's account with secure storage practices.
        Args:

...
def _convert_initial_data_to_uuids(initial_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Use datetime.timezone.utc for consistent UTC timestamps
    ...
    for old_user_email, user_data in converted_data.get("users", {}).items():
        ...
        user_data["email"] = f"user_{user_uuid[:8]}@oxytail.com" # Make email more realistic/dummy
    ...
    for old_channel_id, channel_data in converted_data.get("channels", {}).items():
        ...
        else: # Assign to a random user if owner_id is missing or invalid
            if new_users:
        ...
        # Temporarily store original subscriber IDs for later mapping
        if "subscribers" in channel_data:
            channel_data["subscribers_temp"] = channel_data["subscribers"]
            channel_data["subscribers"] = [] # Clear for now

        # Temporarily store original video IDs for later mapping
        if "videos" in channel_data:
            channel_data["videos_temp"] = channel_data["videos"]
            channel_data["videos"] = [] # Clear for now
        
        # Temporarily store original playlist IDs for later mapping
        if "playlists" in channel_data:
            channel_data["playlists_temp"] = channel_data["playlists"]
            channel_data["playlists"] = [] # Clear for now
    ...
    for old_video_id, video_data in converted_data.get("videos", {}).items():
        ...
        # Temporarily store original 'liked_by' list for later mapping
        if "liked_by" in video_data:
            video_data["liked_by_temp"] = video_data["liked_by"]
            video_data["liked_by"] = [] # Clear for now

        # Temporarily store original comment IDs for later mapping
        if "comments" in video_data:
            video_data["comments_temp"] = video_data["comments"]
            video_data["comments"] = [] # Clear for now
    ...
    for old_playlist_id, playlist_data in converted_data.get("playlists", {}).items():
        ...
        # Temporarily store original video_ids for later mapping
        if "video_ids" in playlist_data:
            playlist_data["video_ids_temp"] = playlist_data["video_ids"]
            playlist_data["video_ids"] = [] # Clear for now
    ...
    # --- Update References in all data structures with new UUIDs ---
    ...
    # Prioritize current_user/channel that were originally set, then pick from existing
    if converted_data.get("current_user") in _initial_user_id_map:
        ...
        converted_data["current_user"] = random.choice(list(new_users.keys()))
    else:
    ...
    if converted_data.get("current_channel") in _initial_channel_id_map:
        ...
        converted_data["current_channel"] = random.choice(list(new_channels.keys()))
    else:
...
# Define bios/descriptions for videos
bios = [
...
RAW_DEFAULT_STATE = {
    "current_user": "alice.smith@oxytail.com", # Will be converted to UUID
    ...
    "users": {
        "alice.smith@oxytail.com": {
            ...
            "email": "alice.smith@oxytail.com", # Will be updated to dummy email based on UUID
            ...
            "watch_later_playlist": [], # New key, will be populated
            "notification_settings": {"comments": True, "subscriptions": True, "likes": False},
            "channel_history": [], # To store channels visited
        ...
        "bob.jones@oxytail.com": {
            ...
            "email": "bob.jones@oxytail.com", # Will be updated to dummy email based on UUID
        ...
        "charlie.brown@oxytail.com": {
            ...
            "email": "charlie.brown@oxytail.com", # Will be updated to dummy email based on UUID
    ...
    "channels": {
        "UC_AliceVlogs": {
            ...
            "owner_id": "alice.smith@oxytail.com", # Will be converted to UUID
            "created_at": datetime.datetime(2023, 2, 1, 12, 0, 0, tzinfo=datetime.timezone.utc),
            "subscribers": ["bob.jones@oxytail.com", "charlie.brown@oxytail.com"], # Will be converted to UUIDs
        ...
        "UC_BobTech": {
            ...
            "owner_id": "bob.jones@oxytail.com", # Will be converted to UUID
            "created_at": datetime.datetime(2022, 12, 10, 9, 30, 0, tzinfo=datetime.timezone.utc),
            "subscribers": ["alice.smith@oxytail.com", "charlie.brown@oxytail.com"], # Will be converted to UUIDs
        ...
        "UC_CharlieCooks": {
            ...
            "owner_id": "charlie.brown@oxytail.com", # Will be converted to UUID
            "created_at": datetime.datetime(2024, 4, 1, 10, 0, 0, tzinfo=datetime.timezone.utc),
            "subscribers": ["alice.smith@oxytail.com"], # Will be converted to UUIDs
    ...
    "videos": {
        "vid_001": {
            ...
            "uploader_id": "alice.smith@oxytail.com", # Will be converted to UUID
            ...
            "liked_by": [] # Will be populated by users' liked_videos
        },
        "vid_002": {
            ...
            "uploader_id": "bob.jones@oxytail.com", # Will be converted to UUID
        ...
        "vid_005": {
            ...
            "uploader_id": "charlie.brown@oxytail.com", # Will be converted to UUID
    ...
    "comments": {
        "comment_001": {
            ...
            "author_id": "bob.jones@oxytail.com", # Will be converted to UUID
        ...
        "comment_002": {
            ...
            "video_id": "vid_001", # Will be converted to UUID
            "author_id": "charlie.brown@oxytail.com", # Will be converted to UUID
        ...
        "comment_003": {
            ...
            "author_id": "alice.smith@oxytail.com", # Will be converted to UUID
...
for i in range(num_users_to_add):
    ...
    all_user_uuids.append(user_id) # Add to list for future connections

    new_user_data = {
        "user_id": user_id,
        "display_name": f"{first} {last}",
        "email": EmailStr(email),
        "joined_date": generate_random_iso_timestamp(days_ago_min=365*2, days_ago_max=365*5), # Joined 2-5 years ago
        "channels": [],
        "subscriptions": [],
        "watch_history": [],
        "liked_videos": [],
        ...
        "account_status": "active" if random.random() < 0.95 else "suspended" # 5% chance of suspended
    }
...
for user_id in all_user_uuids:
    ...
    # Ensure each user has at least one channel, if they don't already
    if not user_data["channels"] and random.random() < 0.8: # 80% chance new users get a channel
        ...
        new_channel_data = {
            ...
            "owner_id": user_id,
            "created_at": generate_random_iso_timestamp(days_ago_min=30, days_ago_max=730), # Created 1 month to 2 years ago
            "subscribers": [], # Will be populated later
            "videos": [], # Will be populated later
            "playlists": [], # Will be populated later
...
# Populate channel subscriptions (after all channels are created)
for user_id in all_user_uuids:
    user_data = DEFAULT_STATE["users"][user_id]
    num_subscriptions = random.randint(0, min(10, len(all_channel_uuids) - 1))
    
    # Select channels the user doesn't own
    ...
    user_data["subscriptions"] = list(set(user_data["subscriptions"])) # Remove duplicates

    # Update subscriber counts for subscribed channels
...
for channel_id, channel_data in DEFAULT_STATE["channels"].items():
    ...
    num_videos_to_add = random.randint(1, 15) # Each channel gets 1-15 new videos
    ...
    for i in range(num_videos_to_add):
        ...
        # Corrected line: 'bios' is now defined
        ...
        video_data = {
            ...
            "published_at": generate_random_iso_timestamp(days_ago_min=0, days_ago_max=730), # Published recently to 2 years ago
            "duration_seconds": random.randint(60, 1800), # 1 min to 30 mins
            "views": random.randint(100, 500000),
            "likes": random.randint(0, 15000),
            "dislikes": random.randint(0, 500),
            "comments": comments_for_video, # Will be populated with comment UUIDs
            "tags": random.sample(channel_niches[random.randint(0, len(channel_niches)-1)]["tags"] + ["new", "viral", "challenge"], random.randint(1, 4)),
            "category": channel_data["channel_type"].replace('_', ' ').title(), # Derive from channel type
            "privacy_status": random.choice(["public", "unlisted", "private"]),
            "age_restricted": random.random() < 0.05, # 5% chance
        ...
        # Update uploader's liked_videos/watch_history (simulate some self-engagement)
        user_owner_data = DEFAULT_STATE["users"][owner_id]
        if random.random() < 0.3: # 30% chance owner likes their own video
        ...
        if random.random() < 0.7: # 70% chance owner watches their own video
            if video_uuid not in user_owner_data["watch_history"]:
                user_owner_data["watch_history"].append(video_uuid)

# Populate users' watch history and liked videos (across all videos)
for user_id in all_user_uuids:
    user_data = DEFAULT_STATE["users"][user_id]
    ...
    user_data["watch_history"] = list(set(user_data["watch_history"])) # Remove duplicates
    ...
    user_data["liked_videos"] = list(set(user_data["liked_videos"])) # Remove duplicates

    # Update video's liked_by list based on user's liked_videos
    for liked_vid_id in user_data["liked_videos"]:
        if liked_vid_id in DEFAULT_STATE["videos"] and user_id not in DEFAULT_STATE["videos"][liked_vid_id]["liked_by"]:
            ...
            DEFAULT_STATE["videos"][liked_vid_id]["likes"] += 1 # Update video's like count

    # Populate watch_later_playlist for some users
    if random.random() < 0.5: # 50% chance a user has a watch later playlist
    ...
    # Populate channel_history for some users (channels they have visited)
...
for user_id in all_user_uuids:
    ...
    if not user_channels: # Skip if user has no channels
        continue
    
    # Create playlists for some of the user's channels
    num_playlists_for_user = random.randint(0, 3) # Each user can have 0-3 new playlists

    for _ in range(num_playlists_for_user):
        playlist_uuid = str(uuid.uuid4())
        channel_for_playlist = random.choice(user_channels) # Associate with one of the user's channels
        
        num_videos_in_playlist = random.randint(1, min(15, len(DEFAULT_STATE["channels"][channel_for_playlist]["videos"])))
        
        # Select videos from the associated channel
        ...
        playlist_data = {
            ...
            "created_at": generate_random_iso_timestamp(days_ago_min=0, days_ago_max=365), # Created up to 1 year ago
...
for video_id, video_data in DEFAULT_STATE["videos"].items():
    ...
    possible_commenters = [uid for uid in all_user_uuids if uid != video_data["uploader_id"]] # Don't let uploader comment on their own video
    ...
    for _ in range(num_comments_to_add):
        ...
        comment_data = {
            ...
            "created_at": generate_random_iso_timestamp(days_ago_min=0, days_ago_max=300), # Comments up to 300 days old

from audio_gorilla.AmazonApis import AmazonApis, DEFAULT_STATE, User, EmailStr
import unittest
...
class TestAmazonApis(unittest.TestCase):
    ...
    def test_show_profile_success(self):
        ...
        result = self.amazon_api.show_profile(EmailStr("user1@example.com"))
        self.assertTrue(result["profile_status"])
        self.assertEqual(result["user_profile"]["first_name"], "Alice")
        self.assertEqual(result["user_profile"]["email"], "user1@example.com")

    def test_show_profile_not_found(self):
        """Test showing the profile of a non-existent user."""
        result = self.amazon_api.show_profile(EmailStr("nonexistent@example.com"))
    ...
    def test_show_account_success(self):
        ...
        self.assertEqual(result["user_account"]["email"], "user1@example.com")
    ...
    def test_delete_account_success(self):
        ...
        self.assertNotIn("user1@example.com", self.amazon_api.users)
    ...
    def test_add_product_to_cart_success(self):
        ...
        self.assertEqual(self.amazon_api.users["user1@example.com"]["cart"][2], 1)
    ...
    def test_add_product_to_cart_insufficient_stock(self):
        ...
        self.assertNotIn(1, self.amazon_api.users["user1@example.com"]["cart"])
    ...
    def test_update_product_quantity_in_cart_success(self):
        ...
        self.assertEqual(self.amazon_api.users["user1@example.com"]["cart"][2], 3)
    ...
    def test_delete_product_from_cart_success(self):
        ...
        self.assertTrue(result["delete_status"])
        self.assertNotIn(2, self.amazon_api.users["user1@example.com"]["cart"])
    ...
    def test_place_order_success(self):
        ...
        initial_order_count = len(self.amazon_api.users["user1@example.com"]["orders"])
        ...
        self.assertEqual(len(self.amazon_api.users["user1@example.com"]["orders"]), initial_order_count + 1)
        self.assertEqual(self.amazon_api.users["user1@example.com"]["cart"], {}) # Cart should be empty
    ...
    def test_show_orders_no_orders(self):
        ...
        self.amazon_api.users["newuser@example.com"] = deepcopy(DEFAULT_STATE["users"]["user1@example.com"])
    ...
    def test_add_payment_card_success(self):
        ...
        initial_card_count = len(self.amazon_api.users["user1@example.com"]["payment_cards"])
        ...
        self.assertTrue(result["add_status"])
        self.assertEqual(len(self.amazon_api.users["user1@example.com"]["payment_cards"]), initial_card_count + 1)
    ...
    def test_delete_payment_card_success(self):
        ...
        self.assertTrue(result["delete_status"])
        self.assertEqual(len(self.amazon_api.users["user1@example.com"]["payment_cards"]), initial_card_count - 1)
        self.assertNotIn(1, self.amazon_api.users["user1@example.com"]["payment_cards"])
    ...
    def test_add_address_success(self):
        ...
        initial_address_count = len(self.amazon_api.users["user1@example.com"]["addresses"])
        ...
        self.assertTrue(result["add_status"])
        self.assertEqual(len(self.amazon_api.users["user1@example.com"]["addresses"]), initial_address_count + 1)
    ...
    def test_delete_address_success(self):
        ...
        self.assertTrue(result["delete_status"])
        self.assertEqual(len(self.amazon_api.users["user1@example.com"]["addresses"]), initial_address_count - 1)
        self.assertNotIn(1, self.amazon_api.users["user1@example.com"]["addresses"])
    ...
    def test_subscribe_prime_monthly_success(self):
        ...
        initial_prime_sub_count = len(self.amazon_api.users["user1@example.com"]["prime_subscriptions"])
        ...
        self.assertEqual(len(self.amazon_api.users["user1@example.com"]["prime_subscriptions"]), initial_prime_sub_count + 1)
        self.assertIn("prime_subscription_id", result)
        new_sub_id = result["prime_subscription_id"]
        self.assertEqual(self.amazon_api.users["user1@example.com"]["prime_subscriptions"][new_sub_id]["plan"], "monthly")
    ...
    def test_search_add_to_cart_and_checkout(self):
        ...
        self.assertEqual(self.amazon_api.users["user1@example.com"]["cart"][mouse_product_id], 1)
    ...
    def test_add_address_and_place_order(self):
        ...
        order_details = self.amazon_api.users["user1@example.com"]["orders"][order_id]
    ...
    def test_add_to_wishlist_then_move_to_cart_and_checkout(self):
        ...
        self.assertEqual(self.amazon_api.users["user1@example.com"]["wish_list"][1], 1)
        initial_laptop_stock = self.amazon_api.products[1]["stock"]
        ...
        self.assertNotIn(1, self.amazon_api.users["user1@example.com"]["wish_list"]) # Should be removed from wishlist
        self.assertEqual(self.amazon_api.users["user1@example.com"]["cart"][1], 1) # Should be in cart

        # 3. Place the order
        place_order_result = self.amazon_api.place_order(payment_card_id=1, address_id=1, user=self.user1)
        self.assertTrue(place_order_result["order_status"])
        self.assertEqual(self.amazon_api.users["user1@example.com"]["cart"], {})
        self.assertEqual(self.amazon_api.products[1]["stock"], initial_laptop_stock - 1)
    ...
    def test_initiate_return_and_show_returns(self):
        ...
        initial_return_count = len(self.amazon_api.users["user1@example.com"].get("returns", {}))
        ...
        self.assertEqual(len(self.amazon_api.users["user1@example.com"]["returns"]), initial_return_count + 1)

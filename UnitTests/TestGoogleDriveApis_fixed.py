...
class TestGoogleDriveApis(unittest.TestCase):
    ...
    FIXES APPLIED:
    1. Changed pageSize -> page_size (API uses snake_case)
    2. Changed status checks to match actual API return keys:
       - retrieval_status for get/list operations
       - status for create/copy operations  
       - delete_status for delete operations
       - star_status for star operations
       - trash_status for trash operations
       - share_status for share operations
       - reset_status for reset operations
    3. Changed result.get("user") -> result.get("user_info")
    """
    ...
    def test_copy_file_success(self):
        """Test successfully copying an existing file."""
        # First create a file to copy
        create_result = self.drive_api.create_file(name="original_file.txt", mimeType="text/plain", content="test content", user_id=self.user_id_alice)
        ...
        if create_result.get("status"):
            ...
            # Copy the file
            copy_result = self.drive_api.copy_file(original_file_id, name="copied_file.txt", user_id=self.user_id_alice)
            self.assertTrue(copy_result.get("status", False))
            
            if copy_result.get("status"):
                copied_file = copy_result["file"]
                self.assertIn("id", copied_file)
                self.assertEqual(copied_file["name"], "copied_file.txt")
                self.assertEqual(copied_file["mimeType"], "text/plain")
                # Verify the copied file is different from the original
